...
export function ProjectConfigs({ project }: { project: Project }) {
  ...
  const configurations = isAspire
    ? [
        {
          ...project.configs[0],
          tag: "2 BHK",
          title: "2 BHK",
          desc: "A practical family home with space for bedrooms, everyday living and a little room to breathe. The brochure includes multiple 2 BHK plan variants.",
        },
        {
          ...project.configs[1],
          tag: "3 BHK",
  ...
  return (
    <section className="wrap py-[clamp(56px,6vw,96px)]">
      <div className="grid items-end gap-8 md:grid-cols-2">
        <div>
          <Heading
            eyebrow={isAspire ? "THE HOMES" : "Configurations"}
            title="2 & 3 BHK Homes"
          />
        </div>
        ...
        <p className="text-lg leading-relaxed text-ink-2 md:pb-1 md:pl-6">
          {isAspire
            ? "Not every family needs the same home. Aspire offers 2 & 3 BHK formats with multiple plan options, so you can compare what fits your family now — and what will still make sense a few years from now."
            : `Every ${project.name} home is planned for cross ventilation and morning light, with a kitchen that sees the living room and a balcony deep enough to hold two chairs and a table. Sanctioned layouts are published before booking.`}
        </p>
      </div>
          ...
          <Reveal key={c.title} delay={i * 0.1}>
            <article className="group relative flex h-full flex-col overflow-hidden rounded-3xl bg-ink text-white shadow-[0_10px_50px_-20px_rgba(16,16,16,0.5)] transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_30px_70px_-30px_rgba(226,1,15,0.35)]">
              ...
              <div className="relative -mt-12 flex flex-1 flex-col px-6 pb-6">
                <h3 className="text-2xl font-medium tracking-[-0.015em] text-white">
                  {c.title}
                </h3>
      ...
      {isAspire && (
        <p className="mt-8 max-w-3xl text-sm leading-relaxed text-ink-2">
          <span className="font-semibold text-ink">Plan Note:</span>{" "}
          The brochure includes multiple plan variants across Blocks A–D. Use
          the actual floor plans and current availability when shortlisting.
        </p>
      )}
    </section>
  );
